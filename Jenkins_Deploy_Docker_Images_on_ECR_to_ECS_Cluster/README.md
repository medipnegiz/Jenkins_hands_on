...
pipeline {
    ...
    stages {
        stage('Build Docker Image') {
            steps {
                sh 'docker build --force-rm -t "$ECR_REGISTRY/$APP_REPO_NAME:latest" .'     // "--force-rm" ara konteynirlari silmek icin kullanilir
                sh 'docker image ls'
            }
        }
    ...
    post {
        always {
            ...
            sh 'docker image prune -af'             // tüm imagelari silmek icin kullanilir
        }
    }
}
```
...
```groovy
  pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {                  // her "stage" de yeni bir shell actigi icin burada tekrar giris yapiliyor
                sh 'aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin "$ECR_REGISTRY"'          
...
{
	"family": "to-do-app",
	"networkMode": "awsvpc",            // disaridan erisilebilir yapar. Kubernetes service gibi.
...
```bash
aws ecs create-service --cluster to-do-app --service-name to-do-app-service --task-definition to-do-app --desired-count 1 --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[subnet-04b4cb38979],securityGroups=[sg-016bca4cba2],assignPublicIp=ENABLED}" # change the subnets and security group and make sure that the 3000 port is open. 
```
