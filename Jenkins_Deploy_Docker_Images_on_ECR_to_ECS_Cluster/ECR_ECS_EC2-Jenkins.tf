...
locals {
  github-email    = "edip@---.com"        # you need to change this line
  github-username = "medipnegiz"          # you need to change this line
  github-token    = "xxxxxxxxxxxxxxx"     # you need to change this line
  key_pair        = "xxxx"                # you need to change this line
  pem_key_address = "xxxx.pem"            # you need to change this line
  user            = "edipnegiz"
}
...
resource "aws_iam_role" "roleforjenkins" {
  name                = "ecr_jenkins_permission-${local.user}"
...
resource "aws_instance" "jenkins-server" {
  ...
  provisioner "local-exec" {
    command = "ssh -i ${local.pem_key_address}.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@${aws_instance.jenkins-server.public_ip} 'sudo cat /var/lib/jenkins/secrets/initialAdminPassword' > initialpasswd.txt"
  }
